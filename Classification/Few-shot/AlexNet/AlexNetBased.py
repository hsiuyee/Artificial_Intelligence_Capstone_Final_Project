...
def train(model, train_ld, test_ld, device,
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='max', factor=0.5, patience=5, min_lr=2e-6
    )
    ...
    for ep in range(1, epochs+1):
        ...
        if ep % 20 == 0:
            ...
            plot_all_metrics(history, out_path='./Classification/Few-shot/AlexNet/', ep=ep)
        if test_acc > best_acc:
            best_acc, no_improve = test_acc, 0
            torch.save(model.state_dict(), './Classification/Few-shot/AlexNet/Alex_fewshot.pth')
        """
        else:
            no_improve += 1
...
if __name__ == '__main__':
    ...
    plot_all_metrics(history, out_path='./Classification/Few-shot/AlexNet/', ep='fin')

    ...
    def __getitem__(self, idx):
        ...
        if self.type == 'train':
            ...
            if lab == 0:
                ...
                if idx1 == idx2:
                    idx2 = (idx1 + 1) % 5
                base1 = self.positive[idx1] if idx % 2 else self.negative[idx1]
                path = os.path.join(self.img_dir, base1 + '.bmp')
                img1 = Image.open(path).convert('RGB')
                base2 = self.positive[idx2] if idx % 2 else self.negative[idx2]
                ...
                path = os.path.join(self.img_dir, base1 + '.bmp')
                img1 = Image.open(path).convert('RGB')
                base2 = self.negative[idx2] if idx % 2 else self.positive[idx2]
                path = os.path.join(self.img_dir, base2 + '.bmp')
                img2 = Image.open(path).convert('RGB')
            if self.transform:
...
def criterion(x1, x2, label, margin: float = 4):
    """
...
def train(model, train_ld, test_ld, device,
          epochs=100, lr=2e-4, patience=10, margin=3):
    ...
    for ep in range(1, epochs+1):
        ...
        with torch.no_grad():
            for x1, x2, y in tqdm(test_ld, desc=f"Epoch {ep}/{epochs} [Test ]"):
                x1, x2, y = x1.to(device), x2.to(device), y.to(device)
                dist = torch.nn.functional.pairwise_distance(model(x1), model(x2))
                preds = (dist >= margin)
        ...
        if ep % 20 == 0:
            ...
            plot_all_metrics(history, out_path='./Classification/Few-shot/ResNet18/', ep=ep)
        if test_acc > best_acc:
            best_acc, no_improve = test_acc, 0
            torch.save(model.state_dict(), './Classification/Few-shot/ResNet18/Res18_fewshot.pth')
        """
        else:
            no_improve += 1
...
if __name__ == '__main__':
    ...
    plot_all_metrics(history, out_path='./Classification/Few-shot/ResNet18/', ep='fin')

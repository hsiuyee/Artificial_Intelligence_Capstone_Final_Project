...
def build_model():
    ...
    model.load_state_dict(torch.load("./Classification/One-shot/ResNet18/Res18_oneshot.pth", weights_only=True))
    return model
...
def valid(model, train_ld, test_ld, device, test_ds):
    ...
    with open("./Classification/One-shot/ResNet18/roc_record.txt", "w") as file:
        for n in fpr:
            file.write(f"{n}, ")
        file.write("\n")
        for n in tpr:
    ...
    plt.tight_layout()
    plt.savefig("./Classification/One-shot/ResNet18/ROC.png")
